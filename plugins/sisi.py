...
def sisi_game(message):
    ...
    if user_data["last_play"] == today:
        bot.reply_to(message, f"{name} 🐣, ты уже играл сегодня! Твой размер груди: {user_data['size']} см")
        return
    ...
    bot.reply_to(message, f"{name} 🎀, твой размер груди вырос на {delta} см. Теперь он равен {user_data['size']} см")