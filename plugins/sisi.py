...
def handle(bot, message):
    ...
    if cmd in ("/sisi", "сиськи"):
        ...
        new_size = top_plugin.load_users(chat)[str(user.id)]["sisi"]
        bot.reply_to(
            message,
...
def handle_successful(bot, message):
    ...
    new_size = top_plugin.load_users(chat_id)[str(user.id)]["sisi"]

    bot.send_message(
        chat_id,
        f"{get_name(user)}, твои сисечки выросли на +{n}, теперь твоя грудь {new_size} размера 😳🍒"
    )