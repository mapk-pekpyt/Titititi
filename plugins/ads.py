...
ADMIN_ID = 5791171535
PRICE_PER_SHOW = 1  # можно изменить через /priser
...
def handle_priser(bot, message):
    ...
    data = load()
    if len(parts) < 2:
        bot.send_message(message.chat.id, f"Текущая цена за 1 показ: {data.get('price', PRICE_PER_SHOW)} звезд")
        return
    try:
        price = float(parts[1])
        data['price'] = price
