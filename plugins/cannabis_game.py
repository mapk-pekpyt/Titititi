...
def update_user(user_id, field, delta):
    cursor.execute(f"""
        UPDATE cannabis SET {field} = MAX(0,{field}+?) WHERE user_id=?
    """, (delta, str(user_id)))
    conn.commit()

def set_time(user_id, field):
    cursor.execute(f"""
        UPDATE cannabis SET {field}=? WHERE user_id=?
    """, (datetime.now().isoformat(), str(user_id)))
    conn.commit()

def can_use_timer(last_time, hours=1):
    if not last_time:
        return True
...
def handle(bot, message):
    ...
    if text.startswith("купить"):
        ...
        top_plugin.update_stat("all", user, "bushes", n)
    ...
    # -------- ПРОДАТЬ ТРАВУ --------
    if text.startswith("продать ") and "кексы" not in text:
        n = int(text.split()[1])
        if u[4]<n:
...
# ================== ТОП ==================
def load_users():
    cursor.execute("SELECT user_id,name,coins,bushes,weed,cakes,joints,hunger,high FROM cannabis")
