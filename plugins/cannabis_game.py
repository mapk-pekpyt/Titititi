...
# Обновляем все NULL money на 1000 (на всякий случай)
cursor.execute("UPDATE cannabis SET money = 1000 WHERE money IS NULL")
conn.commit()
...
def add(user_id, field, value):
    ...
    cursor.execute(
        f"UPDATE cannabis SET {field} = COALESCE({field},0) + ? WHERE user_id=?",
        (value, str(user_id))
    )
    conn.commit()
